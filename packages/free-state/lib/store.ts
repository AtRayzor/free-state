...
export interface Store<T extends AllowedStoreState> {
  ...
  /**
   * Returns a readonly proxy object that represents the current state.
   * Any changes to the store's internal snapshot changes will be
   * automatically available.
   *
   * Any attempts to assign to properties on this object will throw.
   *
   * @returns An immutable proxy of the state.
   */
  getReadonlyProxy(): Readonly<T>;

  /**
   * Returns a static snapshot of the current state.
   *
  ...
  /**
   * Replaces the current state of the store with the given object.
   *
